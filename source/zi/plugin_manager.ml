...
module Getters = struct
  ...
  let make_interface_checker (name, plugin) =
    let module Plugin = (val plugin : Plugin.PLUGIN) in
    match Plugin.make_interface_checker with
    | Some x -> Some (name, Plugin.version, x)
    | None -> None
...
module Resolver = struct
  ...
  let make_interface_checker =
    find_module "MakeInterfaceChecker" Getters.make_interface_checker
...
let resolve_compiler_steps regdescr =
  ...
  let module MakeInterfaceChecker =
  (val Resolver.make_interface_checker !register)
  in
  ...
  let module Steps = struct
    ...
    module Typechecker =
      Typechecker_wrapper.WrapTypechecker
        (LexerAndParser)
        (MakeInterfaceChecker)
        (MakeTypechecker)
