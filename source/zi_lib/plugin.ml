...
module type MAKE_REGISTER_ALLOCATOR = functor (T : COMPILER_TOOLBOX) ->
  REGISTER_ALLOCATOR

module type MAKE_CALLCONV = functor (T : COMPILER_TOOLBOX) -> CALLCONV

module type MAKE_CONSTANT_FOLDING = functor (T : COMPILER_TOOLBOX) ->
  CONSTANT_FOLDING

module type MAKE_DEAD_CODE_ELIMINATION = functor (T : COMPILER_TOOLBOX) ->
...
module type MAKE_TYPECHECKER = functor (T : TYPECHECKER_TOOLBOX) -> TYPECHECKER
...
module type PLUGIN = sig
  ...
  val make_interface_checker : (module MAKE_INTERFACE_CHECKER) option
