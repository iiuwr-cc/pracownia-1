...
module type INTERFACE_CHECKER = sig
  val check_interface : Ast.module_interface -> Types.checked_interface
end

module type TYPECHECKER = sig
  val check_module : Ast.module_definition -> node2type
end
...
module type TYPECHECKER_TOOLBOX = sig
  ...
  val find_and_check_interface :
    loc:Ast.location -> string -> Types.checked_interface
end

module type TYPECHECKER_STEP = sig
  val check_module :
    Ast.module_definition ->
    (node2type, Typechecker_errors.type_checking_error list) result
end
